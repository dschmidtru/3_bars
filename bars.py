...
def get_closest_bar(data, longitude, latitude):
    closestBar = {'Address':'','AdmArea':'','District':'','Name':'','SeatsCount':0, 'longitude':0,'latitude':0}
    barItogInfo = []
    num = 0
    minDist = 0
    while num < (len(data))-1:
        if num ==0:
            ...
            dist = calcDistance(longitude,latitude,data[num]['Cells']['geoData'] ['coordinates'][0],data[num]['Cells']['geoData'] ['coordinates'][1])
            if dist == minDist:
                closestBar = {}
                closestBar['Address'] = data[num]['Cells']['Address']
                closestBar['AdmArea'] = data[num]['Cells']['AdmArea']
...
def printingData(str,data):
    ...
    if len(data) > 1:
        ...
        print('')
        if 'distance' in data[0]:
            strp = "Ближайший бар на расстоянии %.2f метров. Название бара: %s расположен по адресу %s %s %s. Имеет %d seats посадочных мест\n" % (data[0]['distance'],data[0]['Name'],data[0]['Address'],data[0]['AdmArea'],data[0]['District'],data[0]['SeatsCount'])
            print (strp)
        else:
